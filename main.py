...
pipeline_path = os.path.join("pickle", "model.pkl")
with open(pipeline_path, "rb") as f:
    pipeline = pickle.load(f)


def prepare_input_data(form_data):
    furnishing_map = {"not-furnished": 0, "semi-furnished": 1, "furnished": 2}
    ...
    data = {
        ...
        "furnishingstatus": [
            furnishing_map.get(form_data.get("furnishingstatus", "not-furnished"), 0)
        ],
    }
...
def index():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "index.html", form_data=form_data, prediction=prediction_formatted
        )
